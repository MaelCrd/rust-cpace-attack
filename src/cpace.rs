...
pub struct CPace {
    ...
    p: RistrettoPoint, // This is the public point (U or V) derived as p_base * r
    p_base: RistrettoPoint,
    r: Scalar,
}
...
pub struct Step1Out {
    ...
    pub actual_p_base: RistrettoPoint,
    pub actual_r_scalar: Scalar,
}
...
impl CPace {
    fn new<T: AsRef<[u8]>>(
        ...
        let h = st.finalize();
        let p_base_val = RistrettoPoint::from_uniform_bytes(&h); // The base point g^pw
        let mut r_bytes = [0u8; 64];
        getrandom(&mut r_bytes)?;
        let r_val = Scalar::from_bytes_mod_order_wide(&r_bytes);
        let p = p_base_val * r_val; // The actual point sent over the wire (U or V)
        Ok(CPace {
            session_id,
            p,
            // #[cfg(test)] // Store p_base for testing/simulation
            p_base: p_base_val,
            r: r_val,
        })
    }
    ...
    pub fn step1<T: AsRef<[u8]>>(
        ...
        Ok(Step1Out {
            // #[cfg(test)]
            actual_p_base: ctx.p_base.clone(),
            // #[cfg(test)]
            actual_r_scalar: ctx.r.clone(),
            ctx,
            step1_packet,
        })
    }
